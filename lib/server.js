...
const debug = require('debug')('peerjs:server')
...
app._initializeWSS = function(server) {
  ...
  debug("Setting up socket server on path",path)
...
app._configureWS = function(socket, key, id, token) {
  ...
  if (token !== client.token) {
    self._log("Token "+token+" was taken, re-using");
    client.token = token;
  }
...
app._initializeHTTP = function() {
  ...
  this.get('/:key/:id/onlineusers', function(req, res, next) {
    debug("GET /:id/onlineusers",req.params)
  ...
  this.get('/:key/peers', function(req, res, next) {
    debug("GET /:key/peers",req.params)
    var key = req.params.key;
  ...
  var handle = function(req, res, next) {
    debug("POST handler /"+req.path,req.params)
    var key = req.params.key;
    var id = req.params.id;
...
app._setCleanupIntervals = function() {
  ...
  // Clean up outstanding messages every 5 seconds - probably a bit keen - relaxed back to 30s now
  setInterval(function() {
...
app._handleTransmission = function(key, message) {
  ...
  let destination
  if (this._clients && this._clients[key] && this._clients[key][dst]) {
